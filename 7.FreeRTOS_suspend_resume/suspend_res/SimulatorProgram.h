...
int Sim_PWM_Old4 = 0;
int Sim_PWM_Old5 = 0;
int Sim_servo1_OLD = 0;
int Sim_servo2_OLD = 0;
int Sim_Data0;
int Sim_Data1;
int Sim_Value0;
int Sim_Value1;
...
///////////////////////////////////////// Functie DHT11 ///////////////////////////////////////////
int DHTread11(int pin){
   serialEvent();
   int result;
   result = 0;
   if (pin > 1){
     result = Sim_Temp;
     DHTtemperature = String(Sim_Temp, DEC);
     DHThumidity = String(Sim_Hum, DEC);
   }
   return result; 
}
 
///////////////////////////////////////// Functie attach servo /////////////////////////////////////////// 
int myservo1attach(int pin){
    if (pin <= 9) {
      Servo1Pin = String("S0"+String(pin));
     }else {
      Servo1Pin = String("S"+String(pin)); 
     }
}
int myservo2attach(int pin){
    if (pin <= 9) {
      Servo2Pin = String("S0"+String(pin));
     }else {
      Servo2Pin = String("S"+String(pin)); 
     }
}

///////////////////////////////////////// Functie servo1 ///////////////////////////////////////////
int myservo1write(int servo){
    String Sim_Data0 = String(Servo1Pin + String (servo));
    if (servo != Sim_servo1_OLD){      // Nieuw <> Oud
        Sim_servo1_OLD = servo;
        Serial.print(Sim_Data0); 
        delay(20);  
        Serial.print('\n'); 
        delay(10);  // pause 10ms
...
///////////////////////////////////////// Functie servo2 ///////////////////////////////////////////
int myservo2write(int servo){
  String Sim_Data0 = String(Servo2Pin + String (servo));
  if (servo != Sim_servo2_OLD){      // Nieuw <> Oud
        Sim_servo2_OLD = servo;
        Serial.print(Sim_Data0); 
        delay(20);  
        Serial.print('\n'); 
        delay(10);  // pause 10ms
...
int lcdsetCursor(int kolom , int rij){
  ...
  Serial.print('\n'); 
  delay(30);  // pause 10ms  
  String Sim_Data0 = String (kolom);
  String Sim_Data1 = String (kolom);
  if (rij == 0) {
...
int lcdprint(int text){                          // Versturen van een integer
...
int lcdprint(char text){                          // Versturen van een integer
